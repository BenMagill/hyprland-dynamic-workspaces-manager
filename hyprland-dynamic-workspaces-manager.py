...
def get_all_workspaces():
    ...
    for line in workspaces.split("\n"):
        ...
        if regex_groups != None:
            print(line)
            if "special:minimum" not in line:
...
def window_switcher():
    ...
    all_windows_unfiltered = json.loads( json_result )
    all_windows = []
    for window in all_windows_unfiltered:
        if "special:minimum" in window["workspace"]["name"]:
            continue
        else:
